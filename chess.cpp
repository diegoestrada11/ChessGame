...
void callBack(Interface* pUI, void* p)
{
   ...
   {
      // CASE 1: No piece “picked up” yet. first click = pick up a piece
         ...
         else
         {
            ...
            for (const Move& m : legalMoves)
            {
               ...
               if (m.getSource() == posSource && m.getDest() == posSelect)
               {
                  ...
                  // -------------------------------------------
                  // KING‐CAPTURE CHECK: if we captured a KING → game over
                  // -------------------------------------------
                  if (captured == KING)
                  {
      ...
      // Update prevSelect so repeated clicking the same square won’t retrigger
